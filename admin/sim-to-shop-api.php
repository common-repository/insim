...
if (!class_exists('Sim_To_Shop_API')) {

    include_once 'sim-to-shop-reports.php';
    require_once 'API_SMS_PHP_SimToShop/sim-to-shop_web_services.inc.php';
    require_once 'API_SMS_PHP_SimToShop/sms.inc.php';

    class Sim_To_Shop_API {

        private static $sim_to_shop_api;
        public $_phone = null;
        public $_recipients = null;
        public $sms_type = inSIM_SMS_WORLD; // ou encore SMS_STANDARD,SMS_PREMIUM
        public $sms_mode = inSIM_DIFFERE; // ou encore DIFFERE
        //for campaign
        private $_recipient;
        ...
        public static function get_instance() {
            global $sim_to_shop_api;
            if (is_null($sim_to_shop_api)) {
                $sim_to_shop_api = new Sim_To_Shop_API(get_option('solo_sms_email'), get_option('solo_sms_key'));
            }
            return $sim_to_shop_api;
        }

        /**
         * Sim_to_shop user login
         * @var string
         * @access public
         * @since 1.0.0
         */
        ...
        public function get_account() {
            ...
            //echo get_option('solo_sms_email');die();
            if (WP_DEBUG)
                error_log("sim-to-shop - get_account ,parameters $this->user_login , $this->api_key");
        ...
        /**
         * Get thenews on ardary server
         * @return string|boolean
         */
        public function get_news() {
            if (WP_DEBUG)
                    error_log("sim-to-shop - get_news ");
            $sms = new SMS();            
        ...
        public function get_sms_default_text($hook_key,$bAdmin=false) {
            ...
            switch ($hookText) {
                case 'action_create_account_admin':
                    $defaultMessage = __("{firstname} {lastname} has just registered on {shopname}", 'insim');
                    break;
                case 'action_send_message_admin' :
                    $defaultMessage = __("{from} has sent a message to{contact_name} ({contact_mail}) : {message}", 'insim');
                    break;
                case 'action_validate_order_admin' :
                    $defaultMessage = __("New order from {firstname} {lastname}, id: {order_id}, payment: {payment}, total: {total_paid} {currency}.", 'insim');
                    break;
                case 'action_update_quantity_admin' :
                  $defaultMessage = __("This item is almost out of order, id: {product_id}, ref: {product_ref}, name: {product_name}, quantity: {quantity}", 'insim');
                  break; 
				case 'action_test_sms_admin' :
                  return $defaultMessage = __("This is a test from your Woocomerce store. It works!", 'insim');
                  break; 
                case 'action_admin_alert_admin' :
                    $defaultMessage = __("Your SMS credit is almost empty. Your remaining balance is {balance} € available.", 'insim');
                    break;
                case 'action_daily_report_admin' :
                    $defaultMessage = __("date: {date}, inscriptions: {subs}, orders: {orders}, sales: {day_sales}, for the month of: {month_sales}", 'insim');
                    break;
                case 'action_create_account' :
                    $defaultMessage = __("{firstname} {lastname}, Welcome to {shopname} !", 'insim');
                    break;
                case 'action_password_renew' :
                    $defaultMessage = __("{firstname} {lastname}, your new password to access {shopname} is : {password}. {shopurl}", 'insim');
                    break;
                case 'action_customer_alert' :
                    $defaultMessage = __("{firstname} {lastname}, the item {product} is now available on {shopname} ({shopurl})", 'insim');
                    break;
                case 'action_send_message' :
                    $defaultMessage = __("Thank you for your message. We will answer you shortly. {shopname}", 'insim');
                    break;
                case 'action_validate_order' :
                    $defaultMessage = __("{firstname} {lastname}, we do confirm your order {order_id}, of {total_paid} {currency}. Thank you. {shopname}", 'insim');
                    break;
                default:
                    if (strstr($hook_key, 'action_order_status_update')) {
                        $defaultMessage = __('{firstname} {lastname}, your order {order_id} on {shopname} has a new status : {order_state}','insim');
                    } else {
                        $defaultMessage = __('Not defined', 'insim');
        ...
        public function _get_action_admin_alert_values($bSimu = false, $b_admin = false, $params = null) {
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_admin_alert_values - BEGIN');
            if ($bSimu) {
                $values = array(
                ...
                $alert_level = (int) get_option('sim_to_shop_admin_alert');
                $balance = $this->get_balance();
                if (WP_DEBUG)
                    error_log("_get_action_admin_alert_values alert_level $alert_level , balance: $balance");
                if ((get_option('sim_to_shop_admin_alert_sent') == null || (int) get_option('sim_to_shop_admin_alert_sent') != 1) && $alert_level > 0 && $balance !== false && $balance !== '001' && (float) $balance <= (float) $alert_level) {
                    //remember we send the alert message
                    update_option('sim_to_shop_admin_alert_sent', 1);
            ...
            if (WP_DEBUG)
                error_log('_get_action_admin_alert_values ' . print_r($values, true) . ' sim_to_shop_admin_alert_sent:' . get_option('sim_to_shop_admin_alert_sent'));
        ...
        function _get_action_order_status_update_values($bSimu = false, $b_admin = false, $params = array()) {
            if (WP_DEBUG)
                error_log('sim-to-shop - __get_action_order_status_update_values - BEGIN ');
            if (WP_DEBUG)
                error_log('sim-to-shop - __get_action_order_status_update_values - POST ' . print_r($_POST, true) . ' params:' . print_r($params, true));
            ...
            if (WP_DEBUG)
                error_log('sim-to-shop - __get_action_order_status_update_values - values ' . print_r($values, true) . ' phone:' . print_r($this->_phone, true));
        ...
        public function _get_action_validate_order_values($bSimu = false, $b_admin = false, $params = null) {
            ...
            if ($bSimu) {
                ...
                $payment_method = !empty($order->payment_method) ? $order->payment_method : __('undefined', 'sim_to_shop');
                if ($payment_method) {
                ...
                $values = array(
                    '{firstname}' => isset($_POST['billing_first_name']) ? wc_clean($_POST['billing_first_name']) : __('undefined', 'sim_to_shop'),
                    '{lastname}' => isset($_POST['billing_last_name']) ? wc_clean($_POST['billing_last_name']) : __('undefined', 'sim_to_shop'),
                    '{order_id}' => $params['order_id'],
                    '{payment}' => $payment_method,
                    '{total_paid}' => isset($order) ? $order->get_total() : __('undefined', 'sim_to_shop'),
                    '{currency}' => $currency
                );
            }
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_validate_order_values - values ' . print_r($values, true) . ' phone:' . print_r($this->_phone, true));
            return array_merge($values, self::_getBaseValues());
        }

        /**
         * If the option is not free (sim_to_shop_freeoption == 0), verify the order contains the good product_id (sim_to_shop_option_id_product) to send the sms
         * @param type $order_id
         * @param type $b_admin
         * @return boolean
         */
        function can_send_optional_sms($order_id, $b_admin) {
            if (!$b_admin && (int) get_option('sim_to_shop_freeoption') == 0) {
                $option_product_id = get_option('sim_to_shop_option_id_product');
                $b_find = false;
                $order = wc_get_order($order_id);
                if (isset($order)) {
        ...
        public function _get_action_password_renew_values($bSimu = false, $b_admin = false, $params = array()) {
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_password_renew_values - BEGIN');
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_password_renew_values - POST' . print_r($_POST, true));
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_password_renew_values - params:' . print_r($params, true));
            if ($bSimu) {
                $values = array(
                    '{firstname}' => 'John',
                    '{lastname}' => 'Doe',
            ...
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_password_renew_values - values ' . print_r($values, true));
            return array_merge($values, self::_getBaseValues());
        }
        ...
        public function  _get_action_test_sms_values($bSimu = false, $b_admin = false, $params = array()) {
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_test_sms_admin_values - BEGIN');
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_test_sms_admin_values - POST ' . print_r($_POST, true));
            if ($bSimu) {
                $values = array(
                ...
                if (WP_DEBUG)
                    error_log('sim-to-shop - _get_action_test_sms_admin_values - values ' . print_r($values, true));
            }
            return array_merge($values, self::_getBaseValues());
        }
        //action_update_quantity
        public function _get_action_update_quantity_values($bSimu = false, $b_admin = false, $params = array()) {
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_update_quantity_values - BEGIN');
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_update_quantity_values - POST ' . print_r($_POST, true));
            if ($bSimu) {
                $values = array(
                    '{product_id}' => '000001',
                    '{product_ref}' => 'REF-001',
                ...
                if (WP_DEBUG)
                    error_log('sim-to-shop - _get_action_update_quantity_values - values ' . print_r($values, true));
            }
            return array_merge($values, self::_getBaseValues());
        }
        ...
        private function _get_action_send_message_values($bSimu = false, $b_admin = false, $params = array()) {
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_send_message_values - BEGIN');
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_send_message_values - POST ' . print_r($_POST, true));
            ...
            if ($bSimu) {
                ...
                error_log('sim-to-shop - _get_action_send_message_values - POST ' . print_r($params, true));
        ...
        function _get_action_daily_report_values($bSimu = false, $b_admin = false, $params = array()) {
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_daily_report_values - BEGIN');
            if (WP_DEBUG)
                error_log('sim-to-shop - _get_action_daily_report_values - POST' . print_r($_POST, true));
            ...
            if ($bSimu) {
                ...
                $filter['period'] = 'month';
                $reports = new Sim_To_Shop_Reports();
        ...
        public function send($hook, $params) {
            ...
            //     Sim_To_Shop_Admin::get_instance()->action_admin_alert();
            // }
        ...
        public function _prepare_sms($hook_id, $params, $b_admin = false) {
            ...
            if (method_exists(__CLASS__, $method)) {
                ...
                $is_active = get_option(Sim_To_Shop_Admin::get_instance()->_get_isactive_hook_key($hook_id, $b_admin, $params));
                
                //get the text
                $text = get_option(Sim_To_Shop_Admin::get_instance()->_get_hook_key($hook_id, $b_admin, $params));
                if (WP_DEBUG)
                    error_log('sim-to-shop| ' . Sim_To_Shop_Admin::get_instance()->_get_hook_key($hook_id, $b_admin, $params) . ' , text:' . $text . ' , isActive:' . $is_active);
                $locale = get_locale();
                ...
                if ($is_active && $text) {
                    ...
                    //if (is_array($values)) {
                        ...
                        if ($this->_is_everything_valid_for_sending()) {
                            ...
                            $this->_send_sms($text_to_send, $params);
                        } else {
        ...
        public function _set_phone($phone = null, $country = null, $b_admin = false) {
            $this->_phone = null;
            if ($b_admin) {
                $this->_phone = array(get_option('sim_to_shop_admin_phone'));
         ...
         * @return boolean
         */
        public function _send_sms($sms_text, $params) {
            if (WP_DEBUG)
                error_log('_send_sms ' . $sms_text . ' phone:' . print_r($this->_phone, true));
            $sms = new SMS();

            $sms_type = inSIM_SMS_WORLD; //SMS_STANDARD; // ou encore SMS_STANDARD,SMS_PREMIUM
            $sms_mode = inSIM_INSTANTANE; // ou encore DIFFERE
            $sms_sender = get_option('solo_sms_sender');
            ...
            if ($xml = simplexml_load_string($xml)) {
                if (!key_exists('error_code', $xml) || $xml->error_code == '000') {
                    ...
                    error_log(Sim_To_Shop_Admin::get_instance()->get_error_SMS($xml->error_code));
                    return array('error' => true, 'error_code' => $xml->error_code, 'error_details'=> Sim_To_Shop_Admin::get_instance()->get_error_SMS($xml->error_code));
                }
            }
        ...
        public function convert_phone_to_international($phone, $iso_country, $prefix = null) {
            ...
            if (is_null($prefix))
                $prefix = $wpdb->get_var("SELECT prefix FROM `" . $wpdb->prefix . "simtoshop_phone_prefix` WHERE `iso_code` = '" . $iso_country . "'");
            if (empty($prefix))
                return null;
            else {
        ...
        public function validate_campaign($ticket, $time) {
            ...
            $sms->set_sms_mode(inSIM_DIFFERE);
        ...
        /**
         * Get the campaign status on sim_to_shop.
         * 
         * @param type $id_campaign
         * @param type $time
         * @return type
        ...
        /**
         * Cancel a campaign on ardary server if it is possible.
         * @param type $id_campaign
         * @return type
         */
