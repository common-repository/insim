...
if (!class_exists('Sim_To_Shop_Recipient')) {

    class Sim_To_Shop_Recipient {
        ...
        public function validate() {
            foreach (self::$fields as $attr => $field) {
                if (isset($this->$attr) && isset($field['validate'])) {
                    ...
                    if (!$valid) {
                        $this->errors[] = array($attr => $attr . __(" no valid data is set", 'insim'));
                    }
                }
                if (isset($field['required']) && $field['required'] && !isset($this->$attr)) {
                    $this->errors[] = array($attr => __("value is required", 'insim'));
                }
                if (isset($field['size']) && isset($this->$attr) && strlen($this->$attr) > $field['size']) {
                    $this->errors[] = array($attr => __("maximum size is ", 'insim') . ' ' . $field['size']);
                }
        ...
        public static function get_recipient($id_sendsms_campaign, $phone) {
            global $wpdb;
            $sql = "select * from " . Sim_To_Shop_Recipient::_get_table() . " where id_sendsms_campaign = " . (int) $id_sendsms_campaign . " and phone = '" . $phone . "'";
            if (WP_DEBUG)
                error_log($sql);
        ...
        public static function _get_table() {
            global $wpdb;
            return $wpdb->prefix . 'simtoshop_recipient';
        }
